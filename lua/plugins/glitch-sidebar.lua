-- Glitch sidebar: não é um plugin externo, carrega direto via VimEnter
vim.api.nvim_create_autocmd("VimEnter", {
  callback = function()
        ...
        for r = 1, h do
          ...
          if dist >= 0 and dist < col.len then
            ...
            elseif dist < col.len then
              grid[r][c] = math.random() > 0.3 and rand_char(katakana) or rand_char(syms)
              bright[r][c] = 1
            end
          end
        end
    ...
    local function open_panel(side)
      ...
      vim.wo[p.win].foldcolumn = "0"
      vim.wo[p.win].cursorline = false
      vim.wo[p.win].wrap = false
